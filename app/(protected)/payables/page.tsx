...
import { PayableAddForm } from '@/components/payables/PayableAddForm';
...
export default function PayablesPage() {
  ...
  const handleDelete = async () => {
    if (!user || !modalState.isOpen) return;

    const confirmDelete = window.confirm(
      `Are you sure you want to delete "${modalState.payable.accountName}"?`
    );
    if (!confirmDelete) return;

    await deleteDoc(doc(db, `users/${user.uid}/payables/${modalState.payable.id}`));
    await fetchPayables();
    closeModal();
  };
      ...
      <div className="p-6 max-w-3xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <Button
            ...
            color="primary"
            className="shadow-xl/30 hover:shadow-xl/80 hover:scale-[1.03] transition-all duration-1000 cursor-pointer"
          >
            + Add Payable
          </Button>
        </div>
        ...
        <div className="grid gap-4">
          {payables.map((payable) => (
            <div
              key={payable.id}
              className="p-4 shadow-xl/30 hover:shadow-xl/80 hover:scale-[1.03] transition-all duration-1000 cursor-pointer border border-border rounded-lg mb-2"
              onClick={() =>
            ...
            >
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Category: {payable.category}</p>
                  <p className="text-sm text-muted-foreground">Due: {payable.dueDate}</p>
                </div>
        ...
        {/* Modal with pay/edit/delete tabs */}
        {modalState.isOpen && (
          <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center px-4">
            <div className="bg-default rounded-xl shadow-xl w-full max-w-md p-6 relative">
              ...
              {modalState.activeTab === 'edit' && modalState.payable && (
                <PayableAddForm
                  payable={modalState.payable}
                  onSuccess={handleSuccess}
                />
